...
function createUniqueHash (identifier) {
  if (identifier) { identifier = identifier + '_' } else { identifier = '' }
  var hash = ''
  ...
  } else { return createUniqueHash(identifier) }
}

function extractFileName (fileName) {
  var fileNameSplit = fileName.split('/')
  var name = fileNameSplit[fileNameSplit.length - 1]
  if (name === '') { name = fileNameSplit[fileNameSplit.length - 2] }
  return name
}
...
function normaliseFileName (fileName) {
  if (fileName.charAt(0) === '.') { fileName = process.cwd() + '/' + fileName }
  ...
  if (isDirectory(fileName) && fileName.charAt(fileName.length - 1) !== '/') { fileName = fileName + '/' }
...
function updateTest (test, testReults, type) {
  if (!Array.isArray(testReults)) { testReults = [testReults] }
...
function updateParsedSuite (testSuiteName, suite, test) {
  ...
  if (test.type !== 'passed' && test.type !== 'skipped') { parsedSuites[testSuiteName].type = 'failure' }
  ...
  if (!parsedSuites[testSuiteName].hasOwnProperty('testCases')) { parsedSuites[testSuiteName].testCases = [] }
...
function parseTests (suites, fileName) {
  suites.forEach(function (suite) {
    if (!suite.hasOwnProperty('testcase')) { suite.testcase = [] }

    var suiteName
    if (suite.$ && suite.$.name) { suiteName = suite.$.name }
    ...
    suite.testcase.forEach(function (testcase) {
      ...
      if (testcase.passed) { updateTest(test, testcase.passed, 'passed') } else if (testcase.failure) { updateTest(test, testcase.failure, 'failure') } else if (testcase.error) { updateTest(test, testcase.error, 'error') } else if (testcase.skipped) { updateTest(test, testcase.skipped, 'skipped') } else { updateTest(test, testcase, 'passed') }

      var testSuiteName = 'NO CLASSNAME OR SUITE NAME'
      if (testcase.$ && testcase.$.classname) { testSuiteName = testcase.$.classname } else if (suiteName) { testSuiteName = suiteName }
...
function extractSuite (suite, suitesToAdd) {
  ...
  suite.testsuite.forEach(function (childSuite) {
    if (suite.$ && childSuite.$) { childSuite.$.name = suite.$.name + ' ' + childSuite.$.name }
    suitesToAdd.push(childSuite)

    if (childSuite.testsuite) { extractSuite(childSuite, suitesToAdd) }
  })
}
...
function parseSuites (result, fileName) {
  ...
  if (result.testsuites) {
    if (result.testsuites.testsuite) { suites = result.testsuites.testsuite } else if (result.testsuites) { suites = [result.testsuites] }
  } else if (result.testsuite) { suites = [result.testsuite] }
  ...
  var suitesToAdd = []
  suites.forEach(function (suite) {
    if (suite.testsuite) { extractSuite(suite, suitesToAdd) }
  })
...
function parseString (data, fileName) {
  parser.parseString(data, function (err, result) {
    if (err !== null) { parsedSuites[fileName] = createError(fileName, err.toString()) } else if (result === null) { parsedSuites[fileName] = createError(fileName, 'There are no results') } else { parseSuites(result, fileName) }
  })
}

function parseTestResult (fileName) {
  if (!isXml(fileName)) { return }
...
function runThroughFolder (folder) {
  fs.readdirSync(folder).forEach(function (file) {
    ...
    if (isDirectory(fileName)) { runThroughFolder(fileName) } else { parseTestResult(fileName) }
  })
}
...
module.exports = {
  parse: function (fileName, contracted) {
    ...
    parsedSuites.forEach(function (suite) {
      if (!junit_info.suites.hasOwnProperty(suite.type)) { junit_info.suites[suite.type] = 0 }
      junit_info.suites[suite.type] = junit_info.suites[suite.type] += 1
      junit_info.tests.count += suite.testCases.length
      suite.testCases.forEach(function (test) {
        if (!junit_info.tests.hasOwnProperty(test.type)) { junit_info.tests[test.type] = 0 }
